...
def main() -> int:
    ...
    summary = import_workbook(repo, settings, workbook_path)
...
def import_workbook(
    ...
    for student_name, student_id, attendance_date, window_label, stamped_at, distance_m in attendance_sheet.iter_rows(
        min_row=2, values_only=True
    ):
        if student_id is None:
            continue
    ...
    return {
        "database_target": database_target_label(repo),
